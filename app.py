...
API_KEY = os.environ.get("TWELVE_API_KEY", "aab68d4088ec4f7d9762027839651f8b")
...
def refresh_candles():
    ...
    while True:
        try:
            ...
            values = d.get("values", [])
            candles = []
            for v in reversed(values):  # API返回最新在前，翻转为时间正序
                dt_utc = datetime.strptime(v["datetime"], "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
...
def refresh_ratings():
    ...
    while True:
        try:
            r = requests.get(f"{BASE}/analyst_ratings/light", params={
                "symbol": "TSLA",
                "apikey": API_KEY,
                "output_size": 20,
            }, timeout=10)
            d = r.json()
            items = d.get("analyst_ratings_light", d.get("data", []))
            ...
            for item in items:
                ...
                result.append({
                    ...
                    "firm": item.get("analyst", "Unknown"),
                    "from_grade": item.get("rating_change", {}).get("from", "—") if isinstance(item.get("rating_change"), dict) else "—",
                    "to_grade": item.get("rating", item.get("action", "—")),
...
# ── Buy/Hold/Sell 评分（用Twelve Data，1小时刷新）────────────
def refresh_recommendation():
    fail = 0
    while True:
        try:
            r = requests.get(f"{BASE}/recommendations", params={
                "symbol": "TSLA",
                "apikey": API_KEY,
            }, timeout=10)
            d = r.json()
            recs = d.get("recommendations", d.get("data", []))
            if recs:
                latest = recs[0]
                strong_buy  = int(latest.get("strong_buy", 0))
                buy         = int(latest.get("buy", 0))
                hold        = int(latest.get("hold", 0))
                sell        = int(latest.get("sell", 0))
                strong_sell = int(latest.get("strong_sell", 0))
                total = strong_buy + buy + hold + sell + strong_sell
                if total > 0:
                    buy_score  = round((strong_buy * 10 + buy * 7) / total, 1)
                    hold_score = round(hold * 10 / total, 1)
                    sell_score = round((sell * 7 + strong_sell * 10) / total, 1)
                    rec = {
                        "buy": min(buy_score, 10),
                        "hold": min(hold_score, 10),
                        "sell": min(sell_score, 10),
                        "num_analysts": total,
                        "mean": round((strong_buy*1+buy*2+hold*3+sell*4+strong_sell*5)/total, 2),
                    }
                else:
                    rec = None
            else:
...
function renderChart(candles){
  ...
  if(!candles.length){el.innerHTML='<div style="text-align:center;padding:60px;color:#8b949e;font-size:13px">暂无K线数据（数据加载中）</div>';return;}
