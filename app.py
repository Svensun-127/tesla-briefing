...
cache = {
    "quote": {
        "price": "N/A", "change": "N/A", "change_pct": "N/A",
        "is_positive": True, "updated": "",
    },
    "candles": [],
    "news": [],
    "analyst_articles": [],
...
# ── 1. 实时股价（30秒刷新）────────────────────────────────────
# Primary: Yahoo Finance  |  Fallback: Twelve Data
def _refresh_quote():
    fails = 0
    while True:
        ok = False
        ...
        if not ok:
            try:
                r = requests.get(f"{TWELVE_BASE}/quote",
...
def _refresh_candles():
    ...
    while True:
        ...
        try:
            ...
            print(f"[Candles/YF] {len(candles)} bars")
            fails = 0
            ok = True
        except Exception as e:
            print(f"[Candles/YF] {e}")

        # --- Twelve Data fallback ---
        if not ok:
            try:
                r = requests.get(f"{TWELVE_BASE}/time_series", params={
                    "symbol": "TSLA", "interval": "1min",
                    "outputsize": 240, "prepost": "true", "apikey": TWELVE_KEY,
                }, timeout=15)
                ...
                for v in reversed(vals):
                    ...
                    ).replace(tzinfo=timezone.utc).astimezone(BEIJING_TZ)
                    candles.append({
                        "time":   dt_utc.strftime("%m/%d %H:%M"),
                ...
                with _lock:
                    cache["candles"] = candles
                print(f"[Candles/TD] {len(candles)} bars")
                fails = 0
                ok = True
            except Exception as e:
                print(f"[Candles/TD] {e}")
                fails += 1
    ...
    while True:
        try:
            ...
            excluded  = ["fox news", "msnbc", "buzzfeed", "tmz",
                         "daily mail", "new york post", "breitbart"]
            preferred = ["reuters", "bloomberg", "wsj", "barron", "seeking alpha",
                         "electrek", "teslarati", "ars technica", "techcrunch",
                         "marketwatch", "benzinga", "yahoo", "cnbc", "fortune",
                         "business insider", "financial times"]
            keywords  = ["tesla", "tsla", "elon", "musk", "ev ", "electric vehicle"]
                ...
                try:
                    ...
                    for e in feed.entries[:15]:
                        ...
                        src = (e.get("source", {}).get("title")
                               or feed.feed.get("title") or "Unknown")
                        if any(x in src.lower() for x in excluded):
                            continue
                        items.append({
                            "title":     title,
                            "url":       e.get("link", "#"),
                            ...
                            "pref":      any(p in src.lower() for p in preferred),
                        })
                except Exception as ex:
                    print(f"[News] {url}: {ex}")

            items.sort(key=lambda x: not x["pref"])
            result = [{"title": x["title"], "url": x["url"],
...
def _refresh_analyst_articles():
    while True:
        try:
            ...
            kws = [
                "analyst", "price target", "rating", "forecast", "outlook",
                "overweight", "underweight", "upgrade", "downgrade",
                "initiates", "raises", "cuts", "maintains", "research",
                "morgan stanley", "goldman", "jpmorgan", "wedbush",
                "barclays", "ubs", "bank of america", "citi", "deutsche",
                "canaccord", "piper", "bernstein", "jefferies", "cowen",
            ]
            seen, articles = set(), []
            for url in sources:
                try:
                    ...
                    print(f"[Analyst] {url}: {ex}")

            print(f"[Analyst] {len(articles)} articles")
            with _lock:
...
# ── 5. 评级变动 — Yahoo Finance upgradeDowngradeHistory（1小时刷新）
    ...
    while True:
        try:
            ...
            for item in history:
                ...
                if len(result) >= 8:
                    break
            print(f"[Ratings] {len(result)} items")
            with _lock:
...
# ── 6. Elon Musk 推文（Nitter RSS，30分钟刷新）──────────────────
def _refresh_tweets():
    mirrors = [
        ...
        "https://nitter.unixfox.eu/elonmusk/rss",
    ]
    while True:
        try:
                ...
                try:
                    ...
                    for e in feed.entries[:30]:
                        ...
                        if len(tweets) >= 5:
                            break
                    if tweets:
                        print(f"[Tweets] {len(tweets)} from {url}")
                        break
                except Exception as ex:
                    print(f"[Tweets] {url}: {ex}")
            with _lock:
                cache["tweets"] = tweets
        except Exception as e:
            print(f"[Tweets] {e}")
...
# ── 7. Buy/Hold/Sell 评分 — Yahoo Finance recommendationTrend（1小时刷新）
def _refresh_recommendation():
    ...
    while True:
        try:
            ...
            if trends:
                t   = trends[0]
                sb  = int(t.get("strongBuy",  0))
                b   = int(t.get("buy",        0))
                h   = int(t.get("hold",       0))
                s   = int(t.get("sell",       0))
                ss  = int(t.get("strongSell", 0))
                tot = sb + b + h + s + ss
                if tot > 0:
                    rec = {
...
for _i, _fn in enumerate(_workers):
    threading.Thread(target=_run, args=(_fn, _i * 5), daemon=True).start()
