...
# ── 1. 实时股价（30秒刷新）│ 主：Yahoo Finance │ 备用：Twelve Data ────────────
def _refresh_quote():
    fails = 0
    while True:
        ok = False
        ...
        try:
            ...
            prev = round(float(
                meta.get("chartPreviousClose") or meta.get("regularMarketPreviousClose") or meta.get("previousClose") or price
            ), 2)
...
# ── 2. K线图（5分钟刷新）│ 1分钟粒度+盘前盘后 │ 主：Yahoo Finance ────────────
...
# ── 3. 客观/非主流媒体特斯拉新闻（15分钟刷新）──────────────────────────────────
def _refresh_news():
    while True:
        try:
            # 精选：客观媒体+非主流科技/财经源（剔除八卦/流量媒体）
            sources = [
...
# ── 4. 北美顶级投行特斯拉分析文章（15分钟刷新）────────────────────────────────
def _refresh_analyst_articles():
    while True:
        try:
            # 投行核心源：Yahoo财经+特斯拉垂直财经+投行关联财经源
            sources = [
                "https://feeds.finance.yahoo.com/rss/2.0/headline?s=TSLA&region=US&lang=en-US",
                "https://seekingalpha.com/api/sa/combined/TSLA.xml",  # 投行分析核心平台
...
# ── 5. 北美投行特斯拉评级变动（1小时刷新）│ Yahoo Finance 投行评级库 ─────────
...
# ── 6. 马斯克24小时主动推文（30分钟刷新）│ 稳定Nitter镜像+过滤转发 ─────────
def _refresh_tweets():
    # 高可用Nitter镜像（适配Render网络）
    nitter_mirrors = [
            ...
            for url in nitter_mirrors:
                try:
                    ...
                    for e in feed.entries[:30]:
                        ...
                        # 清洗推文内容（移除HTML标签/多余空格）
                        raw = e.get("summary", title)
                        text = re.sub(r"<[^>]+>", "", raw).strip()
                        text = re.sub(r"\s+", " ", text)
                        tweets.append({
                            "text": text[:400],  # 截断过长内容
...
# ── 7. 投行共识Buy/Hold/Sell评分（1小时刷新）│ Yahoo Finance 分析师趋势 ─────
